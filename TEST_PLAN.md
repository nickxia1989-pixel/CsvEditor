...
## Verification Run - 2026-06-07 Keyboard Undo Copy State

- Copy-state hardening: keyboard undo/redo now clears the copied-cell highlight and editor-internal clipboard, matching toolbar undo/redo. This avoids stale copied borders and stale internal paste buffers after `Ctrl+Z`, `Ctrl+Shift+Z`, or `Ctrl+Y` changes sheet state.
- `npm test -- src/components/GridEditor.test.tsx`: 1 file / 53 tests passed after adding a regression where copied A1 is cleared by keyboard `Ctrl+Z` and `Ctrl+Y` through the grid keyboard proxy.
